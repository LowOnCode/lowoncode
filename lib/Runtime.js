...
module.exports = class Runtime {
  ...
  getAllComponentsFlat () {
    // TODO recursive ?
    const data = this.allComponents
    const KEY = 'components'

    const hasChildren = data.filter(elem => elem[KEY] && elem[KEY].length)
    const hasNoChildren = data.filter(elem => !elem[KEY])
  ...
  printComponents () {
    const components = this.getAllComponentsFlat()

    console.table(components.map(elem => ({
      name: elem.name,
